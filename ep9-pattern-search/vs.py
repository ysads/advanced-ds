...
Implements a data structure based on suffix vectors that, given a text T,
allows multiple queries to be made against T *without* reprocessing it.
This uses an OOP approach to make accessing lcp, llcp and rlcp easier –
you don't need to pass them down to every function that needs them.
Instantiating VS is enough to build all helper arrays, you don't need to
call any other function for that.
"""
...
class VS():
  ...
  def search(self, P):
    """
    Given a word P, it looks for P using LCP arrays. If:
    - P is found, returns P.
    - P isn't found, returns the lexicog. predecessor of P in T.
    """
    r = 0
    s = self.suffixes[-1]
  ...
  def print(self, include_lrlcp=False):
    print("\n• Suffixes:")
  ...
  def build_extended_lcp(self):
    """
    Needed to initialize the arrays llcp and rlcp so that we can access whatever
    position we need within lrlcp.
    """
